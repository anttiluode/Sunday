...
def run_gate(seed_start: int = SEED_START, seed_count: int = SEED_COUNT) -> dict:
    rows = [run_seed(seed_start + i) for i in range(seed_count)]
...
def main() -> None:
    ...
    if args.json:
        ...
        m = receipt["metrics"]
        print("Gate 2 — abstract graph attacker")
        ...
        print(f"same-graph coordinate erase     {m['max_graph_same_difference']:.3e}")
        ...
        print(f"rewire interaction signs AB/CD  {m['rewire_AB_interaction_positive_fraction']:.2f}/{m['rewire_CD_interaction_positive_fraction']:.2f}")
        for name, ok in receipt["checks"].items():
            print(f"{'PASS' if ok else 'FAIL'}  {name}")
        print(f"GATE 2: {'PASS' if receipt['gate_pass'] else 'FAIL'}")
